...
class MainViewModel(
    ...
    fun onCancelCaptureClicked() {
        arRenderer?.showGuide = false; arRenderer?.resetAnchor()
        updateState(uiState.value.copy(isCapturingTarget = false, targetCreationState = TargetCreationState.IDLE, captureStep = CaptureStep.ADVICE, capturedTargetImages = emptyList(), targetCreationMode = TargetCreationMode.CAPTURE, isGridGuideVisible = false, arState = ArState.SEARCHING))
    }

    fun onRefinementPathAdded(path: com.hereliesaz.graffitixr.data.RefinementPath) {
        updateState(uiState.value.copy(refinementPaths = uiState.value.refinementPaths + path), isUndoable = true)
    ...
    fun onRefineTargetToggled() {
        if (uiState.value.capturedTargetImages.isNotEmpty()) updateState(uiState.value.copy(isCapturingTarget = true, captureStep = CaptureStep.REVIEW), isUndoable = false)
        else Toast.makeText(getApplication(), "No target captured to refine", Toast.LENGTH_SHORT).show()
    }
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                if (fingerprint.keypoints.isEmpty()) {
                    withContext(Dispatchers.Main) { Toast.makeText(getApplication(), "No features found. Try a more detailed surface.", Toast.LENGTH_LONG).show() }
                    return@launch
                }
                ...
                withContext(Dispatchers.Main) {
                    updateState(uiState.value.copy(isCapturingTarget = false, targetCreationState = TargetCreationState.SUCCESS, isArTargetCreated = true, arState = ArState.SEARCHING, fingerprintJson = Json.encodeToString(Fingerprint.serializer(), fingerprint), refinementPaths = emptyList(), targetMaskUri = null, showImagePicker = uiState.value.overlayImageUri == null))
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) { handleTargetCreationFailure(e) }
            }
        }
    }
    ...
    fun onCreateTargetClicked() {
        ...
        updateState(uiState.value.copy(isCapturingTarget = true, captureStep = CaptureStep.ADVICE, capturedTargetImages = emptyList(), targetCreationMode = TargetCreationMode.CAPTURE, arState = ArState.SEARCHING), isUndoable = false)
    }
    ...
    private fun processCapturedTargets(images: List<Bitmap>) {
        viewModelScope.launch {
            _feedbackEvent.emit(FeedbackEvent.VibrateDouble)
            val segmenter = SubjectSegmentation.getClient(SubjectSegmenterOptions.Builder().enableMultipleSubjects(SubjectSegmenterOptions.SubjectResultOptions.Builder().enableConfidenceMask().build()).build())
                ...
                segmenter.process(InputImage.fromBitmap(frontImage, 0)).addOnSuccessListener { result ->
                    viewModelScope.launch(Dispatchers.IO) {
                        ...
                        result.subjects.firstOrNull()?.let { subject ->
                            ...
                            subject.confidenceMask?.let { buffer ->
                                buffer.rewind(); val pixels = IntArray(subject.width * subject.height)
                                for (i in pixels.indices) { if (buffer.hasRemaining()) { val alpha = (buffer.get() * 255).toInt().coerceIn(0, 255); pixels[i] = (alpha shl 24) or 0x00FFFFFF } }
                                maskBitmap.setPixels(pixels, 0, subject.width, 0, 0, subject.width, subject.height)
                            }
                            maskUri = saveBitmapToCache(maskBitmap, "mask")
                        }
                        arRenderer?.setAugmentedImageDatabase(images)
                        withContext(Dispatchers.Main) { updateState(uiState.value.copy(capturedTargetImages = images, targetMaskUri = maskUri, captureStep = CaptureStep.REVIEW, targetCreationState = TargetCreationState.SUCCESS, isArTargetCreated = true, arState = ArState.SEARCHING, isLoading = false)); updateDetectedKeypoints(); if (uiState.value.overlayImageUri == null) updateState(uiState.value.copy(showImagePicker = true), isUndoable = false) }
                    }
    ...
    private fun handleTargetCreationFailure(e: Exception) {
        updateState(uiState.value.copy(isCapturingTarget = false, targetCreationState = TargetCreationState.IDLE, captureStep = CaptureStep.ADVICE, capturedTargetImages = emptyList(), arState = ArState.SEARCHING, isLoading = false, isGridGuideVisible = false))
    ...
    fun onCycleRotationAxis() {
        val nextAxis = when (uiState.value.activeRotationAxis) { RotationAxis.X -> RotationAxis.Y; RotationAxis.Y -> RotationAxis.Z; RotationAxis.Z -> RotationAxis.X }
        updateState(uiState.value.copy(activeRotationAxis = nextAxis, showRotationAxisFeedback = true)); Toast.makeText(getApplication(), "Rotation Axis: ${nextAxis.name}", Toast.LENGTH_SHORT).show()
    }
    ...
    fun onArImagePlaced() {
        ...
        if (uiState.value.isCapturingTarget && uiState.value.captureStep == CaptureStep.ADVICE) updateState(uiState.value.copy(captureStep = CaptureStep.CHOOSE_METHOD), isUndoable = false)
        else if (uiState.value.overlayImageUri == null) updateState(uiState.value.copy(showImagePicker = true), isUndoable = false)
    }

    fun onProgressUpdate(progress: Float, bitmap: Bitmap? = null) {
        if (abs(progress - uiState.value.progressPercentage) > 5.0f && bitmap != null) { viewModelScope.launch(Dispatchers.IO) { saveBitmapToCache(bitmap, "evolution")?.let { uri -> withContext(Dispatchers.Main) { updateState(uiState.value.copy(evolutionCaptureUris = uiState.value.evolutionCaptureUris + uri), isUndoable = false) } } } }
    ...
    fun onRemoveBackgroundClicked() {
        viewModelScope.launch {
            setLoading(true); (uiState.value.originalOverlayImageUri ?: uiState.value.overlayImageUri)?.let { uri ->
                try {
                    val resultBitmap = BackgroundRemover.removeBackground(BitmapUtils.getBitmapFromUri(getApplication(), uri) ?: return@launch)
                    if (resultBitmap != null) {
                        saveBitmapToCache(resultBitmap, "background_removed")?.let { newUri ->
                            updateState(uiState.value.copy(overlayImageUri = newUri, backgroundRemovedImageUri = newUri, isLoading = false)); updateActiveLayer { it.copy(uri = newUri, backgroundRemovedUri = newUri) }
                        }
                    } else { setLoading(false); Toast.makeText(getApplication(), "Background removal failed", Toast.LENGTH_SHORT).show() }
                } catch (e: Exception) { setLoading(false) }
            } ?: setLoading(false)
        }
    }

    fun onBrightnessChanged(brightness: Float) { updateState(uiState.value.copy(brightness = brightness.coerceIn(-1f, 1f)), isUndoable = false) }

    fun onLineDrawingClicked() {
        viewModelScope.launch {
            setLoading(true); if (uiState.value.isLineDrawing) { (uiState.value.backgroundRemovedImageUri ?: uiState.value.originalOverlayImageUri ?: uiState.value.overlayImageUri)?.let { updateState(uiState.value.copy(overlayImageUri = it, isLineDrawing = false, isLoading = false)); updateActiveLayer { l -> l.copy(uri = it) } } ?: setLoading(false) } else {
                (uiState.value.backgroundRemovedImageUri ?: uiState.value.originalOverlayImageUri ?: uiState.value.overlayImageUri)?.let { uri ->
                    val lineDrawingBitmap = convertToLineDrawing(BitmapUtils.getBitmapFromUri(getApplication(), uri)?.copy(Bitmap.Config.ARGB_8888, true) ?: return@launch, isWhite = true)
                    saveBitmapToCache(lineDrawingBitmap, "line_drawing")?.let { newUri -> updateState(uiState.value.copy(overlayImageUri = newUri, isLineDrawing = true, isLoading = false)); updateActiveLayer { it.copy(uri = newUri) } }
                } ?: setLoading(false)
            }
        }
    }
    ...
    private fun applyCurvesToOverlay(uri: Uri, points: List<Offset>) {
        viewModelScope.launch {
            setLoading(true); try {
                val resultBitmap = applyCurves(BitmapUtils.getBitmapFromUri(getApplication(), uri) ?: return@launch, points)
                saveBitmapToCache(resultBitmap, "curves")?.let { updateState(uiState.value.copy(processedImageUri = it, isLoading = false)) }
            } catch (e: Exception) { setLoading(false) }
        }
    }
    ...
    fun onColorBalanceRChanged(value: Float) { updateState(uiState.value.copy(colorBalanceR = value.coerceIn(0f, 2f)), isUndoable = false) }
    fun onColorBalanceGChanged(value: Float) { updateState(uiState.value.copy(colorBalanceG = value.coerceIn(0f, 2f)), isUndoable = false) }
    fun onColorBalanceBChanged(value: Float) { updateState(uiState.value.copy(colorBalanceB = value.coerceIn(0f, 2f)), isUndoable = false) }
    ...
    fun onMagicClicked() { if (uiState.value.rotationX != 0f || uiState.value.rotationY != 0f || uiState.value.rotationZ != 0f) { updateState(uiState.value.copy(rotationX = 0f, rotationY = 0f, rotationZ = 0f)); Toast.makeText(getApplication(), "Aligned Flat", Toast.LENGTH_SHORT).show() } }
    ...
    private suspend fun performSave(name: String, toast: Boolean) {
        try {
            val project = ProjectData(backgroundImageUri = uiState.value.backgroundImageUri, overlayImageUri = uiState.value.overlayImageUri, originalOverlayImageUri = uiState.value.originalOverlayImageUri ?: uiState.value.overlayImageUri, targetImageUris = if (uiState.value.capturedTargetImages.isNotEmpty()) uiState.value.capturedTargetImages.mapNotNull { saveBitmapToCache(it) } else uiState.value.capturedTargetUris, refinementPaths = uiState.value.refinementPaths, opacity = uiState.value.opacity, brightness = uiState.value.brightness, contrast = uiState.value.contrast, saturation = uiState.value.saturation, colorBalanceR = uiState.value.colorBalanceR, colorBalanceG = uiState.value.colorBalanceG, colorBalanceB = uiState.value.colorBalanceB, scale = uiState.value.scale, rotationZ = uiState.value.rotationZ, rotationX = uiState.value.rotationX, rotationY = uiState.value.rotationY, offset = uiState.value.offset, blendMode = uiState.value.blendMode, fingerprint = uiState.value.fingerprintJson?.let { Json.decodeFromString<Fingerprint>(it) }, drawingPaths = uiState.value.drawingPaths, progressPercentage = uiState.value.progressPercentage, evolutionImageUris = uiState.value.evolutionCaptureUris, gpsData = getGpsData(), sensorData = getSensorData(), calibrationSnapshots = uiState.value.calibrationSnapshots)
            projectManager.saveProject(name, project)
            if (toast) withContext(Dispatchers.Main) { Toast.makeText(getApplication(), "Project '$name' saved", Toast.LENGTH_SHORT).show() }
        } catch (e: Exception) { if (toast) withContext(Dispatchers.Main) { Toast.makeText(getApplication(), "Save failed", Toast.LENGTH_SHORT).show() } }
    }

    private suspend fun getSensorData(): com.hereliesaz.graffitixr.data.SensorData? {
        return suspendCancellableCoroutine { cont ->
            val sm = getApplication<Application>().getSystemService(Context.SENSOR_SERVICE) as? android.hardware.SensorManager ?: return@suspendCancellableCoroutine cont.resume(null)
            val acc = sm.getDefaultSensor(android.hardware.Sensor.TYPE_ACCELEROMETER); val mag = sm.getDefaultSensor(android.hardware.Sensor.TYPE_MAGNETIC_FIELD)
            if (acc == null || mag == null) return@suspendCancellableCoroutine cont.resume(null)
            var g: FloatArray? = null; var m: FloatArray? = null
            val l = object : android.hardware.SensorEventListener {
                override fun onSensorChanged(e: android.hardware.SensorEvent?) {
                    if (e?.sensor?.type == android.hardware.Sensor.TYPE_ACCELEROMETER) g = e.values.clone()
                    if (e?.sensor?.type == android.hardware.Sensor.TYPE_MAGNETIC_FIELD) m = e.values.clone()
                    if (g != null && m != null) {
                        val r = FloatArray(9); val o = FloatArray(3)
                        if (android.hardware.SensorManager.getRotationMatrix(r, null, g, m)) { android.hardware.SensorManager.getOrientation(r, o); sm.unregisterListener(this); if (cont.isActive) cont.resume(com.hereliesaz.graffitixr.data.SensorData(o[0], o[1], o[2])) }
                    }
                }
                override fun onAccuracyChanged(s: android.hardware.Sensor?, a: Int) {}
            }
            sm.registerListener(l, acc, android.hardware.SensorManager.SENSOR_DELAY_NORMAL); sm.registerListener(l, mag, android.hardware.SensorManager.SENSOR_DELAY_NORMAL)
        }
    }
    ...
    fun unwarpImage(points: List<Offset>) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val mat = Mat(); val warped = Mat(); val transform = Mat()
            val srcMat = Mat(4, 2, CvType.CV_32F); val dstMat = Mat(4, 2, CvType.CV_32F)
            ...
            try {
                ...
                val topRow = sortedByY.take(2).sortedBy { it.x }; val bottomRow = sortedByY.takeLast(2).sortedBy { it.x }
                val tl = topRow[0]; val tr = topRow[1]; val bl = bottomRow[0]; val br = bottomRow[1]

                val widthA = sqrt((br.x * w - bl.x * w).pow(2) + (br.y * h - bl.y * h).pow(2))
                val widthB = sqrt((tr.x * w - tl.x * w).pow(2) + (tr.y * h - tl.y * h).pow(2))
                val maxWidth = max(widthA, widthB)
                val heightA = sqrt((tr.x * w - br.x * w).pow(2) + (tr.y * h - br.y * h).pow(2))
                val heightB = sqrt((tl.x * w - bl.x * w).pow(2) + (tl.y * h - bl.y * h).pow(2))
                ...
                if (mat.nativeObj != 0L) mat.release()
                if (warped.nativeObj != 0L) warped.release()
                if (transform.nativeObj != 0L) transform.release()
                if (srcMat.nativeObj != 0L) srcMat.release()
                if (dstMat.nativeObj != 0L) dstMat.release()
                perspectiveTransform?.let { if (it.nativeObj != 0L) it.release() }
            }
        }
    }
