...
- **Dependabot #27 — Netty HTTP header injection in HttpProxyHandler** (`io.netty:netty-handler-proxy`). Fixed by forcing the netty family (incl. `netty-handler-proxy`) to `4.2.17.Final` in `build.gradle.kts` `commonForcedDependencies` (the forced version has since moved past `4.2.15.Final`, which is what was originally verified). **Verified:** `4.2.15.Final` addressed CVE-2025-67735 and subsequent regressions at the time; the current `4.2.17.Final` pin has not been separately re-verified against this CVE list.

- **Dependabot #23/#24/#25 — Bouncy Castle (covert timing channel / LDAP injection / risky crypto algo).** Fixed by forcing `org.bouncycastle:{bcprov,bcpkix,bcutil}-jdk18on:1.86` in `build.gradle.kts` `commonForcedDependencies` (1.84 first patched all three; the pin has since moved to 1.86, a later release that still carries those fixes — the three artifact versions must match).
