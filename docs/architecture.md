...
graph TD
    ...
    I[MainViewModel] -->|UI State| J[Jetpack Compose / AzNavRail]
    J -->|Overlay| H
```
### Algorithm Details

1.  **Voxel Hashing:** Space is divided into 5mm cubes.
2.  **Duplication as Signal:**
...
* **Usage:**
    1.  **Save:** When the user saves, we extract ORB features from the current camera view.
    2.  **Load:** When the user returns, the app runs feature matching on the live feed.
    3.  **Lock:** When > 20 inliers are found (Homography match), we calculate the transform matrix and snap the `world.map` to the physical wall.
