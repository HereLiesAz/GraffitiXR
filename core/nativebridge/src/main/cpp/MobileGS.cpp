...
void MobileGS::relocThreadFunc() {
    ...
    while (mRelocRunning) {
        ...
        for (float s : {0.5f, 2.0f}) {
            ...
            cv::Mat Hback = cv::Mat({3, 3}, CV_64F, {1.0/(double)s, 0.0, 0.0, 0.0, 1.0/(double)s, 0.0, 0.0, 0.0, 1.0}).clone();
        ...
        if (imgPts.size() >= 8) {
            ...
            cv::Mat intr = cv::Mat({3, 3}, CV_64F, {fx, 0.0, cx, 0.0, fy, cy, 0.0, 0.0, 1.0}).clone();
