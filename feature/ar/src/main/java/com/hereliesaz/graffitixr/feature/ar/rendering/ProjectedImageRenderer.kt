...
class ProjectedImageRenderer {
    ...
    fun createOnGlThread(context: Context) {
        val vertexShader = ShaderUtil.loadGLShader(VERTEX_SHADER, GLES30.GL_VERTEX_SHADER, context)
        val fragmentShader = ShaderUtil.loadGLShader(FRAGMENT_SHADER, GLES30.GL_FRAGMENT_SHADER, context)
        ...
        GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER, vertices.size * 4, vertexData, GLES30.GL_STATIC_DRAW)
    ...
    fun draw(viewMtx: FloatArray, projMtx: FloatArray, anchorMatrix: FloatArray, layer: OverlayLayer) {
        if (pendingBitmap != null && !isTextureLoaded) {
            GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, textureId)
