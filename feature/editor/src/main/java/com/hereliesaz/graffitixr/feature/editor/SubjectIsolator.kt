...
/**
 * Cuts the subject out of an image, making everything else transparent — so a projected design does
 * not paint its own background onto the wall.
 *
 * ML Kit's segmenter needs nothing from the app but the bitmap, so this holds no Context; the
 * client is lazy because loading the model is not free and most sessions never isolate.
 */
@Singleton
